...
Page({
  ...
  typeActive(e) {
    ...
    this.setData({
      typeInd: e.currentTarget.dataset.i,
      page_no: 1,
      videosArr: [],
      oldData: [],
      data: []
  ...
  swiperChange1(e) {
    ...
    if (dy) {
      ...
      if (videosArr.length) {
        videosArr.length == 1 ? this.setData({
          Id: videosArr[0].video_id
        }) : '';
        ...
        let t = this.data.Id == data[newcurrent].video_id;
        this.setData({
          t
        })
      }
  ...
  onShow() {
    let {
      ...
      indcurrent,
      globalData
    } = this.data, platform = globalData.systemInfo.platform;
    if (!hidePage) {
      if (platform == "android") {
        this.setData({
          hidePage: true,
  ...
  loadData(first) {
    ...
    http.postReq("/community/video2/", {
      ...
      for (let [key, value] of records.entries()) {
        value.video_id == id ? records.splice(key, 1) : '';
      }
      if (first) {
  ...
  timeupdate(e) {
    ...
    // percent = Math.round(currentTime / duration * 10000) / 100
  ...
  updata(i) {
    ...
    for (let [key,value] of videosArr.entries()) {
      if (value.video_id == i.video_id) {
        videosArr[key]=i;
        break;
      }
